...
function btnClicked() {
  ...
  document.getElementById('d1-ticket').innerHTML = determineViolation(d1Speed, d1Limit);
  document.getElementById('d2-ticket').innerHTML = determineViolation(d2Speed, d2Limit);
  document.getElementById('d3-ticket').innerHTML = determineViolation(d3Speed, d3Limit);

  function determineViolation(speed, limit) {
    let ticket;
    if (speed > limit + 40) {
